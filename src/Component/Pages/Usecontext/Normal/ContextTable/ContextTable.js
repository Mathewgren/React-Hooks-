...
const ContextTable = () => {
  ...
  const filteredData = stateEmp.employees.filter((row) => {
    ...
    return (
      ...
      (row.phoneNumber && row.phoneNumber.toLowerCase().includes(searchText))
    );
  });
  ...
  const clearFilters = () => {
    ...
    dt.current.filter("", "confirmPassword", "equals");
    dt.current.filter("", "language", "equals");
    dt.current.filter("", "phoneNumber", "equals");
  };
      ...
      <div className="datatable">
        ...
        >
          ...
          <Column
            ...
            sortable
            filter
            filterPlaceholder="Search by confirmPassword"
            style={{ width: "25%" }}
          />
          <Column
            field="dob"
