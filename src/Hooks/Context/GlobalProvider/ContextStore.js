...
import reducer from "../Function/ReducerUsecontext";
import {
  postData,
  getUsers,
  getData,
...
export const GlobalProvider = ({ children }) => {
  ...
  const addEmployee = async (employee) => {
    ...
    const res = await postData(employee);
  ...
  const updateEmployees = async (employee) => {
    ...
    const res = await putData(employee.id, employee);
    if (res.status === 200) {
